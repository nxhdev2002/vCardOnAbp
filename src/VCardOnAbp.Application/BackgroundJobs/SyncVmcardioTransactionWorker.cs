...
namespace VCardOnAbp.BackgroundJobs
{
    public class SyncVmcardioTransactionWorker : HangfireBackgroundWorkerBase
    {
        ...
        public override async Task DoWorkAsync(CancellationToken cancellationToken = default)
        {
            ...
            var cards = await _cardRepository.GetActiveCardAsync(Supplier.Vmcardio);
            foreach (var card in cards)
            {
                var trans = await _vmcardioAppService.GetCardTransactions(new GetVmCardTransactionInput
                {
                    start = string.Empty,
                    end = string.Empty,
                    status = string.Empty,
                ...
                var cardTrans = await (await _cardTransactionRepository.GetQueryableAsync())
                    .AsNoTracking()
                    .Where(x => x.CardId == card.Id)
                    .ToListAsync(cancellationToken);

                var transNotInDb = trans
